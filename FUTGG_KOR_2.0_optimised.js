...
// @description  Translate English text on FUT.GG into Korean (Optimized & Natural)
...
(function() {
    ...
    const dynamicTranslations = [
        ...
        // 스쿼드 배틀 특정 조건
        {
            pattern: /Play (\d+) match(?:es)?\s+in\s+Squad\s+Battles\s+on\s+min\.?\s+Semi-Pro\s+difficulty\s+\(or\s+Rush\/Rivals\/Champions\/Live\s+Events\)\s+using\s+your\s+active\s+EVO\s+Player\s+in\s+game/gi,
            replacement: '스쿼드 배틀(최소 세미프로) 또는 러시/라이벌/챔피언스/라이브 이벤트에서 활성화된 진화 선수를 사용해 $1경기를 플레이하세요'
        },
        {
            pattern: /Win (\d+) match(?:es)?\s+in\s+Squad\s+Battles\s+on\s+min\.?\s+Semi-Pro\s+difficulty\s+\(or\s+Rush\/Rivals\/Champions\/Live\s+Events\)\s+using\s+your\s+active\s+EVO\s+Player\s+in\s+game/gi,
            replacement: '스쿼드 배틀(최소 세미프로) 또는 러시/라이벌/챔피언스/라이브 이벤트에서 활성화된 진화 선수를 사용해 $1경기를 승리하세요'
        },

        // Rivals/Champions 전용
        {
            pattern: /Play (\d+) match(?:es)? in (?:Rivals or Champions|Champions or Rivals) using your active EVO Player in game/i,
            replacement: '라이벌 또는 챔피언스에서 활성화된 진화 선수를 사용해 $1경기를 플레이하세요'
        },
        {
            pattern: /Win (\d+) match(?:es)? in (?:Rivals or Champions|Champions or Rivals) using your active EVO Player in game/i,
        ...
        // 하이픈 포함 패턴
        {
            pattern: /\b(CAM|CM|CDM|ST|CF|LW|RW|LM|RM|CB|LB|RB|LWB|RWB|GK)\s*-\s*([A-Za-z\s-]+?)(\+{1,2})?\b/gi,
        ...
        // 스페이스만 있는 패턴
        {
            pattern: /\b(CAM|CM|CDM|ST|CF|LW|RW|LM|RM|CB|LB|RB|LWB|RWB|GK)\s+([A-Za-z\s-]+?)(\+{1,2})?\b(?!\s*-)/gi,
            replacement: (match, position, role, plus = '') => {
                const translatedRole = translateRole(role);
                return `${position} ${translatedRole}${plus}`;
            }
